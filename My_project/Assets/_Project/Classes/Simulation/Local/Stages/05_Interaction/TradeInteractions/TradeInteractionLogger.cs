...
namespace _Project.Scripts.Simulation.Local.Stages.Interaction
{
    internal static class TradeInteractionLogger
    {
        ...
        {
            Debug.Log($"[Trade] {action} start. ship={shipId} station={stationId} item={itemId} amount={amount} price={price}");
        }
        ...
        {
            Debug.Log($"[Trade] {action} failed. ship={shipId} station={stationId} item={itemId} amount={amount} reason={reason}");
        }

        internal static void LogTradeOrderMissing(string side, int shipId, int stationId, int itemId) // лог отсутствующего ордера
        {
            Debug.Log($"[Trade] {side} order missing. ship={shipId} station={stationId} item={itemId}");
        }
    }
}
